...
author: Birgen Haest, Fabian Hertner
...
2.a   Optional reclassification to include bat-type echoes.

2.b   Filter the data to your target output period, class, and other preferences.

3.    Calculate the activity or migration traffic rates
...
classSelection = c("passerine_type", "bat", "insect")
...
#                    "large_bird", "unid_bird", "bird_flock",
#                    "insect", "nonbio", "precipitation","bat")
classSelection.mtr = classSelection
...
Extracting the data that is need for MTR calculations, can be done with the `extractDbData()` function. This function will connect to your SQL database and read all the necessary data to memory. If requested, it will also write it to an .rds file in the output directory you chose. The file is automatically named to: `[yourDbName]_DataExtract.rds`, and written to the main output directory 'mainOutputDir' you defined in the input settings.

If you have no connection to an SQL server with a MR1-database attached, you can skip step 1) and manually load such the RDS-file : 'dbData = readRDS(file.path(mainOutputDir, `[yourDbName]_DataExtract.rds`))'.
...
***
# Optional - Reclass echoes to include bat classification

In addition to the standard echo classification in birds, insects, and non-biological echoes, the MR1 software (version 1.7.0.0 or newer) offers the possibility to classify echoes as bat.

The bat classification is provided independently in separate tables. The function 'reclassToBats' overwrite the primary classification for possible bat-type echoes. 

```{r}
# not Run
# table(dbData$echoData$class)
...
# not Run - check reclassification
# table(dbData$echoData$class)
```
...
# ===========================================================================
plotClass = 'passerine_type' # "allClasses"
