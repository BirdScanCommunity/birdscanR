  ...
  for (cAltBin in altBinIds) {
    ...
    for (i in 1:nPlots) {
      ...
      if (length(mtrPlot[, 1]) > 0) {
        ...
        # =================================================================
        subtitle <- paste0(
          format(mtrPlot$timeChunkDate[1], "%d-%b-%Y"), " to ",
          format(
            mtrPlot$timeChunkDate[length(mtrPlot$timeChunkDate)],
            "%d-%b-%Y"
          ), "\n",
        ...
        savePlotToFile(
          ...
          altitudeRange = c(
            min(mtrPlot$altitudeChunkBegin),
