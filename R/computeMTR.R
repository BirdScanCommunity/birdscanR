...
computeMTR = function(dbName,
  ...
  if (nrow(echoes) == 0) {
    stop(paste0(
      "There are no echoes to calculate the mtr on. Please check your ",
      "input dataset."
    ))
  }
  ...
  if (computePerDayNight == TRUE) {
    ...
    if ("timeChunkDateSunset" %in% colnames(mtr) && !all(is.na(mtr$timeChunkDateSunset))) {
      for (k in 1:length(unique(mtr$altitudeChunkId))) {
        ...
        mtrDay = data.frame(
          timeChunkDate = as.Date(timeChunkDate$x),
        ...
        for (i in 1:length(classSelection)) {
          mtrFirstQuartile = suppressWarnings(
            stats::aggregate(
              mtrTmp[
                mtrTmp$proportionalTimeObserved > propObsTimeCutoff,
      ...
      timeChunks = data.frame(
        timeChunkBegin = unique(mtr$timeChunkBegin),
      ...
      warning(paste0(
        "Compute MTR per day/night not possible. Set parameter ",
  ...
  if (computePerDayCrepusculeNight == TRUE) {
    ...
    if ("timeChunkDateSunset" %in% colnames(mtr) && !all(is.na(mtr$timeChunkDateSunset))) {
      ...
      warning(paste0(
        "Compute MTR crepuscular not possible. Set parameter ",
    ...
    for (i in 0:length(classSelection)) {
      ...
      for (k in 1:nrow(mtr)) {
        ...
        if (mtr$observationTime_h[k] <= 0) {
          ...
          mtr[k, paste("meanAltitude", classLabel, sep = ".")] = stats::weighted.mean(
            x = echoesInTimeAndAltitudeBin$feature1.altitude_AGL,
