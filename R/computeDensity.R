...
computeDensity = function(dbName,
  ...
  if (nrow(echoes) == 0) {
    stop(paste0(
      "There are no echoes to calculate the density on. Please check your ",
      "input dataset."
    ))
  }
    ...
    for (i in 0:length(classSelection)) {
      ...
      for (k in 1:nrow(density)) {
        ...
        if (density$observationTime_h[k] <= 0) {
          ...
          density[k, paste("meanAltitude", classLabel, sep = ".")] = stats::weighted.mean(
