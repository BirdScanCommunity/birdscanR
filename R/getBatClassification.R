...
getBatClassification <- function(dbConnection, dbDriverChar) {
  ...
  if (batClassificationTableExists && batProbabilityTableExists) {
    ...
    batClassificationTable$class <- availableClasses$name[match(
      batClassificationList,
      availableClasses$id
    )]
    ...
    batClassProbabilityTable$class <- availableClasses$name[match(
      batClassList,
      availableClasses$id
    )]
    ...
    if (nrow(batClassProbabilityTable) > 0) {
      classProbabilites <- reshape2::dcast(
        batClassProbabilityTable[!is.na(batClassProbabilityTable$class) &
    ...
    batClassificationTable <- data.frame(
      echo = integer(),
      batClass = character(),
  ...
  return(list(
    batClassificationTable = batClassificationTable,
