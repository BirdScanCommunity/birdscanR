      ...
      if (nrow(echoDataPlot) >= 0) {
        ...
        if (!is.null(manualBlindTimes)) {
          ...
          background <- rbind(
            background,
            data.frame(
              xStart = manualBlindTimes$start_targetTZ,
        ...
        if (!is.null(sunriseSunset)) {
          background <- rbind(
            background,
            data.frame(
              xStart = sunriseSunset$civilStart,
        ...
        if (!is.null(visibilityData)) {
          background <- rbind(
            background,
            data.frame(
              xStart = visibilityData$blind_from_targetTZ,
        ...
        if (!is.null(protocolData)) {
          background <- rbind(
            background,
            data.frame(
              xStart = protocolData$stopTime_targetTZ[1:(length(protocolData[, 1]) - 1)],
        ...
        # ===================================================================
        insects <- echoDataPlot[
          echoDataPlot$class %in% c("insect"),
          names(echoDataPlot) %in% c(
            "time_stamp_targetTZ",
        ...
        # save plot
        # ===================================================================
        plotWidth_mm <- as.numeric(difftime(
          timeRange[[i]][2],
          timeRange[[i]][1],
          "days"
        ) * 30 + 50)
