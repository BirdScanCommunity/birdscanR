...
createTimeBins <- function( timeRange, timeBinDuration_sec, timeZone, sunriseSunset, sunOrCivil = "civil" )
{
  ...
  if( sunOrCivil == "sun" )
  {
    timeBinsDN <- data.frame( start = sunriseSunset$sunStart, stop = sunriseSunset$sunStop, dayOrNight = sunriseSunset$is_night, dateSunset = sunriseSunset$date )
  } else
  {
    timeBinsDN <- data.frame( start = sunriseSunset$civilStart, stop = sunriseSunset$civilStop, dayOrNight = sunriseSunset$is_night, dateSunset = sunriseSunset$date )
  }
  
  # Limit day/night timeBins to timeRange
  timeBinsDN <- timeBinsDN[ timeBinsDN$start >= timeRange[ 1 ] & timeBinsDN$start <= timeRange[ 2 ], ]
  timeBinsDN$dayOrNight[ timeBinsDN$dayOrNight == 1 ] <- "night"
  timeBinsDN$dayOrNight[ timeBinsDN$dayOrNight == 0 ] <- "day"
  
  # create timeBins
  timeStart <- timeRange[ 1 ] - as.numeric( format( timeRange[ 1 ], "%M" ) ) * 60 - as.numeric( format( timeRange[ 1 ], "%S" ) ) - ( 60 * 60 * 24 )
  timeStop <- timeRange[ 2 ] + ( 60 * 60 * 24 )
  sequence <- seq( from = timeStart, to = timeStop, by = timeBinDuration_sec )
  timeBins <- data.frame( start = sequence[ 1 : length( sequence ) - 1 ], stop = sequence[ 2 : length( sequence ) ], dayOrNight = NA, dateSunset = as.POSIXct( NA, tz = timeZone ) )
  timeBins <- timeBins[ timeBins$stop > timeRange[ 1 ] & timeBins$start < timeRange[ 2 ], ]
  ...
  if( sunOrCivil == "sun" )
  {
    nightTimes <- data.frame( start = sunriseSunset[ sunriseSunset$is_night == 1, ]$sunStart, stop = sunriseSunset[ sunriseSunset$is_night == 1, ]$sunStop )
    dayTimes <- data.frame( start = sunriseSunset[ sunriseSunset$is_night == 0, ]$sunStart, stop = sunriseSunset[ sunriseSunset$is_night == 0, ]$sunStop )
  } else if( sunOrCivil == "civil" )
  {
    nightTimes <- data.frame( start = sunriseSunset[ sunriseSunset$is_night == 1, ]$civilStart, stop = sunriseSunset[ sunriseSunset$is_night == 1, ]$civilStop )
    dayTimes <- data.frame( start = sunriseSunset[ sunriseSunset$is_night == 0, ]$civilStart, stop = sunriseSunset[ sunriseSunset$is_night == 0, ]$civilStop )
  } else
  {
    nightTimes <- data.frame( start = sunriseSunset[ sunriseSunset$is_night == 1, ]$sunStart, stop = sunriseSunset[ sunriseSunset$is_night == 1, ]$sunStop )
    dayTimes <- data.frame( start = sunriseSunset[ sunriseSunset$is_night == 0, ]$sunStart, stop = sunriseSunset[ sunriseSunset$is_night == 0, ]$sunStop )
  ...
  isNight <- vapply( timeBinsMean, function( x ) { x >= nightTimes$start & x < nightTimes$stop }, logical( length( nightTimes[ , 1 ] ) ) )
  isNight <- colSums( isNight )
  isDay <- vapply( timeBinsMean, function( x ) { x >= dayTimes$start & x < dayTimes$stop }, logical( length( dayTimes[ , 1 ] ) ) ) 
  isDay <- colSums( isDay )
  ...
  timeBins <- data.frame( id = seq( from = 1, to = length( timeBins[ , 1 ] ), by = 1 ), timeBins )
