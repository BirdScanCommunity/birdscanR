...
createTimeBins = function(timeRange,
  ...
  if (timeRange[1] > timeRange[2]) {
    warning(paste0(
      "End of time range is before begin of time range. Set a ",
  ...
  if (dnBins) {
    if (sunOrCivil == "sun") {
      ...
      timeBinsDN = data.frame(
        start = sunriseSunset$civilStart,
  ...
  if (crepBins) {
    if (crepuscule %in% "civilSolar") {
      timeBinsCrep = data.frame(
        start = sunriseSunset$civilStart[sunriseSunset$is_night == 0],
      ...
      tmpBinsCrep = data.frame(
        start = sunriseSunset$sunStart[sunriseSunset$is_night == 1],
      ...
      timeBinsCrep = rbind(timeBinsCrep, tmpBinsCrep)
      tmpBinsCrep = data.frame(
        start = sunriseSunset$civilStart[sunriseSunset$is_night == 1],
      ...
      timeBinsCrep = data.frame(
        start = sunriseSunset$nauticalStart[sunriseSunset$is_night == 0],
    ...
    if (length(rowsDoubleNA) != 0) {
      ...
      timeBinsCrep$stop[(rowsDoubleNA - 1)] = as.POSIXct(
        rowMeans(data.frame(
          as.numeric(timeBinsCrep$start[(rowsDoubleNA - 1)]),
      ...
      timeBinsCrep$start[(rowsDoubleNA + 1)] = as.POSIXct(
        rowMeans(data.frame(
          as.numeric(timeBinsCrep$start[(rowsDoubleNA - 1)]),
          as.numeric(timeBinsCrep$stop[(rowsDoubleNA + 1)])
        )),
    ...
    if (length(rowsStopNAStartPlusOneNA) != 0) {
      ...
      # ===================================================================
      timeBinsCrep$stop[rowsStopNAStartPlusOneNA] = as.POSIXct(
        rowMeans(data.frame(
          as.numeric(timeBinsCrep$start[rowsStopNAStartPlusOneNA]),
      ...
      # ===================================================================
      timeBinsCrep$start[(rowsStopNAStartPlusOneNA + 1)] = as.POSIXct(
        rowMeans(data.frame(
          as.numeric(timeBinsCrep$start[(rowsStopNAStartPlusOneNA)]),
          as.numeric(timeBinsCrep$stop[(rowsStopNAStartPlusOneNA + 1)])
        )),
  ...
  if (dnBins) {
    timeBins = data.frame(
      start = sequence[1:(length(sequence) - 1)],
  ...
  if (dnBins) {
    if (sunOrCivil == "sun") {
      nightTimes = data.frame(
        start = sunriseSunset[sunriseSunset$is_night == 1, ]$sunStart,
        stop = sunriseSunset[sunriseSunset$is_night == 1, ]$sunStop
      )
      dayTimes = data.frame(
        start = sunriseSunset[sunriseSunset$is_night == 0, ]$sunStart,
        stop = sunriseSunset[sunriseSunset$is_night == 0, ]$sunStop
      )
    } else if (sunOrCivil == "civil") {
      nightTimes = data.frame(
        start = sunriseSunset[sunriseSunset$is_night == 1, ]$civilStart,
        stop = sunriseSunset[sunriseSunset$is_night == 1, ]$civilStop
      )
      dayTimes = data.frame(
        start = sunriseSunset[sunriseSunset$is_night == 0, ]$civilStart,
  ...
  if (dnBins) {
    ...
    isNight = vapply(
      timeBinsMean,
      function(x) {
        x >= nightTimes$start & x < nightTimes$stop
      },
    ...
    isDay = vapply(
      timeBinsMean,
      function(x) {
        x >= dayTimes$start & x < dayTimes$stop
      },
  ...
  if (crepBins) {
    if (crepuscule %in% "civilSolar") {
      ...
      crepusculeMorning = data.frame(
        start = sunriseSunset$civilStart[sunriseSunset$is_night == 0],
        stop = sunriseSunset$sunStart[sunriseSunset$is_night == 0]
      )
      crepusculeEvening = data.frame(
        start = sunriseSunset$sunStart[sunriseSunset$is_night == 1],
        stop = sunriseSunset$civilStart[sunriseSunset$is_night == 1]
      )
      nights = data.frame(
  ...
  if (crepBins) {
    timeBinsMean = timeBins$start + difftime(timeBins$stop, timeBins$start, "secs") / 2
    isCrepMorning = vapply(
      timeBinsMean,
      function(x) {
        x >= crepusculeMorning$start & x < crepusculeMorning$stop
      },
      logical(nrow(crepusculeMorning))
    )
    if(is.vector(isCrepMorning)){isCrepMorning = matrix(isCrepMorning, nrow = 1)}
    isCrepMorning = colSums(isCrepMorning, na.rm = TRUE)
    isCrepEvening = vapply(
      timeBinsMean,
      function(x) {
        x >= crepusculeEvening$start & x < crepusculeEvening$stop
      },
      logical(nrow(crepusculeEvening))
    )
    if(is.vector(isCrepEvening)){isCrepEvening = matrix(isCrepEvening, nrow = 1)}
    isCrepEvening = colSums(isCrepEvening, na.rm = TRUE)
    isNight = vapply(
      timeBinsMean,
      function(x) {
        x >= nights$start & x < nights$stop
      },
      logical(nrow(nights))
    )
    if(is.vector(isNight)){isNight = matrix(isNight, nrow = 1)}
