...
createAltitudeBins <- function( altitudeRange, altitudeBinSize )
{
  ...
  altitudeBins <- data.frame( id = seq( from = 1, to = length( sequence ) - 1, by = 1 ), begin = sequence[ 1 : length( sequence ) - 1 ], end = sequence[ 2 : length( sequence ) ], size = as.numeric( NA ), avgAltitude = as.numeric( NA ) )
