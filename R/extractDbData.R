...
extractDbData <- function(dbDriverChar = "SQL Server",
  ...
  if (is.null(dbName)) {
    stop("dbName is not defined. Please check your input!")
  }
  ...
  rm(
    collectionTable, echoRfFeatureMap,
  ...
  if (is.null(radarTimeZone)) {
    ...
    if (tz_shift >= 0 || tz_shift < 0) {
      radarTimeZone <- paste0(
        ...
        abs(tz_shift)
      ) # note that "UTC+1" is denoted as "Etc/GMT-1"
      message(paste0(
        "Radar time zone extracted from dbo.site is :",
        radarTimeZone
      ))
    }
  }
  ...
  protocolData <- convertTimeZone(
    data = protocolData,
    colNames = c("startTime", "stopTime"),
    originTZ = radarTimeZone,
    targetTZ = targetTimeZone
  )
  siteData <- convertTimeZone(
    data = siteData,
    colNames = c("projectStart", "projectEnd"),
    originTZ = radarTimeZone,
    targetTZ = targetTimeZone
  )
  timeBinData <- convertTimeZone(
    data = timeBinData,
